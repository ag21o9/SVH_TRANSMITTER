...
interface DeviceConfig {
  backend: string;
  ipPortPairs: { id: string; ip: string; port: string; }[];
  deviceImei: string;
  vendorId: string;
...
class HTTPConnectionManager {
  ...
  async sendPacket(
    ...
    port: string, 
    type: 'login' | 'pvt',
    onResponse: (response: PacketResponse) => void
  ) {
  ...
  startTransmission(
    serverConfigs: {ip: string, port: string, id: string}[], 
    deviceConfig: any, 
    onResponse: (response: PacketResponse) => void
  ) {
    ...
    serverConfigs.forEach(async (serverConfig) => {
      const loginPacket = this.buildLoginPacket(deviceConfig);
      await this.sendPacket(loginPacket, serverConfig.ip, serverConfig.port, 'login', onResponse);
    });

    // Set up intervals for PVT packets (every 5 seconds)
    serverConfigs.forEach((serverConfig) => {
      const interval = setInterval(async () => {
        const pvtPacket = this.buildPvtPacket(deviceConfig);
        await this.sendPacket(pvtPacket, serverConfig.ip, serverConfig.port, 'pvt', onResponse);
      }, 5000);
  ...
  private buildPvtPacket(input: any): string {
    const now = new Date();
    ...
    const checksum = this.calculateChecksum(payloadWithoutChecksum);
    return `${payloadWithoutChecksum}*${checksum}`;
  }

  private buildLoginPacket(input: any): string {
    const fields = [
      '$LGN',
      input.vendorId || 'VNDR',
      input.deviceImei,
...
const TransmissionPage: React.FC<TransmissionPageProps> = ({ config, onBack }) => {
  ...
  const [transmissionStartTime, setTransmissionStartTime] = useState<Date | null>(null);
  ...
  useEffect(() => {
    ...
    if (isTransmitting && transmissionStartTime) {
      interval = setInterval(() => {
        setElapsedTime(Math.floor((Date.now() - transmissionStartTime.getTime()) / 1000));
      }, 1000);
    }
    return () => {
  ...
  useEffect(() => {
    if (packetResponses.length > 0) {
      setTimeout(() => {
        scrollViewRef.current?.scrollToEnd({ animated: true });
      }, 100);
    }
  }, [packetResponses.length]);
  ...
  const handleStartTransmission = useCallback(() => {
    try {
      ...
      setTransmissionStartTime(new Date());
      setPacketResponses([]);
  ...
  }, [config, httpManager, handlePacketResponse]);
  ...
  const shareResponses = async () => {
    try {
      const content = packetResponses.map(item => 
        `[${item.timestamp}] ${item.type.toUpperCase()} - ${item.host}:${item.port} - ${item.status.toUpperCase()}\n` +
        `Packet: ${item.packet}\n` +
  ...
  const clearResponses = () => {
    Alert.alert(
      ...
      [
        ...
        { text: 'Clear', style: 'destructive', onPress: () => setPacketResponses([]) }
      ]
    );
  };
  ...
  const getStatusStats = () => {
    const success = packetResponses.filter(r => r.status === 'success').length;
    const error = packetResponses.filter(r => r.status === 'error').length;
    const login = packetResponses.filter(r => r.type === 'login').length;
    const pvt = packetResponses.filter(r => r.type === 'pvt').length;
    return { success, error, login, pvt, total: packetResponses.length };
  };

  const stats = getStatusStats();

  // Start transmission automatically when page loads
  useEffect(() => {
    handleStartTransmission();
    return () => {
      if (httpManager.isTransmitting()) {
        httpManager.stopTransmission();
      }
    };
  }, []);
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          <View style={styles.headerTop}>
            ...
            <View style={styles.headerCenter}>
              <Text style={styles.headerTitle}>Live Transmission</Text>
        ...
        <View style={styles.deviceInfoCard}>
          ...
          <View style={styles.deviceInfoContent}>
            <View style={styles.deviceInfoRow}>
              <Text style={styles.deviceInfoLabel}>IMEI:</Text>
              <Text style={styles.deviceInfoValue}>{config.deviceImei}</Text>
            </View>
            <View style={styles.deviceInfoRow}>
              <Text style={styles.deviceInfoLabel}>Vehicle:</Text>
              <Text style={styles.deviceInfoValue}>{config.vehicleNumber}</Text>
            </View>
            <View style={styles.deviceInfoRow}>
              <Text style={styles.deviceInfoLabel}>Servers:</Text>
              <Text style={styles.deviceInfoValue}>
                {config.ipPortPairs.map(p => `${p.ip}:${p.port}`).join(', ')}
              </Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.responsesContainer}>
          <View style={styles.responsesHeader}>
            <Text style={styles.responsesTitle}>Packet Responses</Text>
            <View style={styles.packetTypeStats}>
              <View style={styles.packetTypeBadge}>
                <Text style={styles.packetTypeText}>LOGIN: {stats.login}</Text>
              </View>
              <View style={styles.packetTypeBadge}>
                <Text style={styles.packetTypeText}>PVT: {stats.pvt}</Text>
              </View>
            </View>
          </View>
          ...
          {packetResponses.length === 0 ? (
            <View style={styles.emptyState}>
              <Icon name="radio" size={48} color="#B0BEC5" />
              <Text style={styles.emptyText}>Waiting for responses...</Text>
            </View>
          ) : (
            <ScrollView
              ref={scrollViewRef}
            ...
            >
              {packetResponses.map((response, index) => (
        ...
        <View style={styles.controlContainer}>
          ...
          >
            <Icon 
              name={isTransmitting ? "stop" : "play-arrow"} 
              size={24} 
              color="#fff" 
            />
            <Text style={styles.controlButtonText}>
              {isTransmitting ? "Stop Transmission" : "Resume Transmission"}
            </Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
  ...
  return (
    <Animated.View style={[
      ...
      <TouchableOpacity 
        ...
        activeOpacity={0.8}
      >
        <View style={styles.responseCardLeft}>
          <View style={[
            styles.responseStatusIcon,
            response.status === 'success' ? styles.statusIconSuccess : styles.statusIconError
          ]}>
            <Icon 
              name={response.status === 'success' ? 'check' : 'close'} 
              size={16} 
              color="#fff" 
            />
          </View>
          <View style={styles.responseCardInfo}>
            <View style={styles.responseCardTopRow}>
              <Text style={styles.responseTimestamp}>{response.timestamp}</Text>
              <View style={[
                styles.packetTypeBadge,
                response.type === 'login' ? styles.loginBadge : styles.pvtBadge
              ]}>
                <Text style={styles.packetTypeText}>{response.type.toUpperCase()}</Text>
              </View>
            </View>
            <Text style={styles.responseServer}>{response.host}:{response.port}</Text>
          </View>
        </View>
        <Icon 
      ...
      {expanded && (
        <View style={styles.responseCardContent}>
          <View style={styles.responseSection}>
            <Text style={styles.responseSectionTitle}>📤 Packet Sent</Text>
          ...
          <View style={styles.responseSection}>
            <Text style={styles.responseSectionTitle}>📥 Server Response</Text>
            <Text style={[
              styles.responseText,
